...
function FollowTask:needToFollow()
    ...
    if self.parent == nil or self.FollowChar == nil or self.FollowChar:getCurrentSquare() == nil then
        return false
    end
...
function FollowTask:update()
    ...
    if (true) then -- self.parent:isInAction() == false) then -- for some reason this is true when they doing nothing sometimes...
        ...
        if (distance < (GFollowDistance + self.FollowDistanceOffset + (Option_FollowDistance / 5))) or
            ...
            if (distance - (Option_FollowDistance / self.FollowDistanceOffset) >=
                (GFollowDistance + self.FollowDistanceOffset)) then
            ...
            elseif (self.FollowChar:getVehicle() ~= nil) and (self.parent:Get():getVehicle() == nil) then
                ...
                if (self.MySeat ~= -1) then
                    ...
                    if (doorsquare ~= nil) then
                        ...
                        ISTimedActionQueue.add(ISEnterVehicle:new(self.parent:Get(), self.FollowChar:getVehicle(),
                            doorseat))
