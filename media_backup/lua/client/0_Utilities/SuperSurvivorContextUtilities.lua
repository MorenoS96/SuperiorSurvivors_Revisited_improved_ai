...
function getSquaresBetween(from, to)
    ...
    repeat
        ...
        if sqr ~= nil then
            debugContext("saving square x : " .. tostring(fromX) .. " y : " .. tostring(fromY) .. " into position : " ..
                             tostring(pos))
            squares[pos] = sqr
            pos = pos + 1
        end
...
function getFleeSquareAlt(fleeGuy, attackGuy, distanceToFlee)
    ...
    local fx, fy = fleeGuy:getX(), fleeGuy:getY()
    local ax, ay = attackGuy:getX(), attackGuy:getY()
    local px, py = player:getX(), player:getY()

    -- Vector away from zombie
    local dx = fx - ax
    local dy = fy - ay
    ...
    -- Blend them (0.7 = mostly away from zombie, 0.3 = slightly towards player)
    local bx = dx * 0.7 + pxv * 0.3
    local by = dy * 0.7 + pyv * 0.3

    -- Normalize to desired flee distance
    local len = math.sqrt(bx * bx + by * by)
    if len == 0 then
        len = 1
    end
    bx = (bx / len) * distanceToFlee
    by = (by / len) * distanceToFlee

    -- Add a little randomness
    local fleex = fx + bx + ZombRand(-3, 3)
    local fleey = fy + by + ZombRand(-3, 3)
...
---@param y number
---@param z number
function getTowardsMiddle(moveguy, x, y, z)
    -- Get current position
    local curX = moveguy:getX()
    local curY = moveguy:getY()
    local curZ = moveguy:getZ()
    ...
    -- Return the grid square at the midpoint
    return moveguy:getCell():getGridSquare(midX, midY, midZ)
end
        ...
        for y = bdefY - 1, bdHeight do
            ...
            if (sq) then
                ...
                for j = 0, Objs:size() - 1 do
                    ...
                    if (instanceof(Object, "IsoWindow")) and (not windowHasBarricade(Object, character)) and
                        (not Object:isLocked()) and (not Object:isPermaLocked()) and distance < closestSoFar then
            ...
            if (sq) then
                ...
                for j = 0, Objs:size() - 1 do
                    ...
                    if (Object ~= nil) then
                        ...
                        if (instanceof(Object, "IsoDoor")) and (Object:isExteriorDoor(character)) and
                            (distance < closestSoFar) then
