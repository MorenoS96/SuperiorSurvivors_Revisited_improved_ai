...
function AIManager(TaskMangerIn)
    ...
    print("AI Manager Running for " .. ASuperSurvivor:getName())
    ...
    if (ASuperSurvivor:getGroupRole() == "Companion") then
        ...
        if ( -- Checks for NPC and main player is indoor/outdoor-like conditions
    ...
    if (AiNPC_Job_Is(NPC, "Companion") and (DistanceBetweenMainPlayer <= 12)) then
        ...
        if ((TaskMangerIn:getCurrentTask() ~= "Attack") and (TaskMangerIn:getCurrentTask() ~= "Threaten") and
            (TaskMangerIn:getCurrentTask() ~= "First Aide") and (Task_Is_Not("Flee")) ---	and (ASuperSurvivor:isInSameRoom(ASuperSurvivor.LastEnemeySeen)) 
        ) and ((ASuperSurvivor:hasWeapon() and
            ((ASuperSurvivor:getDangerSeenCount() >= 1) or
                (ASuperSurvivor:isEnemyInRange(ASuperSurvivor.LastEnemeySeen)))) or
            (ASuperSurvivor:hasWeapon() == false and (ASuperSurvivor:getDangerSeenCount() == 1) and
                (not EnemyIsSurvivor))) -- and ((not ASuperSurvivor:isTooScaredToFight() and (IHaveInjury == false)) -- This. I may want to change this to 'too many injuries' function
        ...
        then
            if (ASuperSurvivor.player ~= nil) and (ASuperSurvivor.player:getModData().isRobber) and
                ...
                print("Companion ATTACK TASK ADDED for companion: " .. ASuperSurvivor:getName())
            end
        end
        ...
        --  NPC:getDangerSeenCount() removed
        -- --------------------------------- --
        if ((NPC:needToReload()) or (NPC:needToReadyGun(weapon))) and (ASuperSurvivor:hasAmmoForPrevGun()) and
            NPC:usingGun() and (ASuperSurvivor:getNeedAmmo()) then
            NPC:ReadyGun(weapon)
        ...
        -- Flee to heal  -- 
        -- -----------   --
        if (TaskMangerIn:getCurrentTask() ~= "Flee") and
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if ((TaskMangerIn:getCurrentTask() ~= "Attack") and (TaskMangerIn:getCurrentTask() ~= "Threaten") and
            not ((TaskMangerIn:getCurrentTask() == "Surender") and EnemyIsSurvivor) and
            (TaskMangerIn:getCurrentTask() ~= "Doctor") and (ASuperSurvivor:isInSameRoom(ASuperSurvivor.LastEnemeySeen)) and
            (TaskMangerIn:getCurrentTask() ~= "Flee") and (TaskMangerIn:getCurrentTask() ~= "Flee From Spot") -- and (NPC:NPC_CheckPursueScore() > 0) -- New: It maybe pursue, but it can be used for attack too, it's helping against door spam
        -- and ( Distance_AnyEnemy < NPC:NPC_CheckPursueScore() ) -- Don't want them chasing from across the map
            ...
            (ASuperSurvivor:hasWeapon() == false and (ASuperSurvivor:getDangerSeenCount() == 1) and
                (not EnemyIsSurvivor))) and (not ASuperSurvivor:isTooScaredToFight() and (IHaveInjury == false)) -- This. I may want to change this to 'too many injuries' function
        and (not NPC:isTooScaredToFight()) and (ASuperSurvivor:inFrontOfLockedDoor() == false) then
            if (ASuperSurvivor.player ~= nil) and (ASuperSurvivor.player:getModData().isRobber) and
                (not ASuperSurvivor.player:getModData().hitByCharacter) and EnemyIsSurvivor and
                (not EnemySuperSurvivor.player:getModData().dealBreaker) then
                TaskMangerIn:AddToTop(ThreatenTask:new(ASuperSurvivor, EnemySuperSurvivor, "Scram"))
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if (IHaveInjury) then
            if (TaskMangerIn:getCurrentTask() ~= "First Aide") and (TaskMangerIn:getCurrentTask() ~= "Flee") and
                ...
                ((NPC:getSeenCount() >= 1) and (Distance_AnyEnemy <= 6)) -- This line doesn't make sense, what if the npc needs to heal outside of hostiles?
            then
                TaskMangerIn:AddToTop(FirstAideTask:new(ASuperSurvivor)) -- If general healing
                TaskMangerIn:AddToTop(FleeTask:new(ASuperSurvivor))
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then -- To ABSOLUTELY prevent these two jobs from listening to this task.
        if (TaskMangerIn:getCurrentTask() ~= "Flee") and (TaskMangerIn:getCurrentTask() ~= "Flee From Spot") and
            ...
            ((TaskMangerIn:getCurrentTask() ~= "Surender") and not EnemyIsSurvivor) and
    ...
    -- ----------------------------- --
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if (IsInAction == false) and (ASuperSurvivor:getNeedAmmo() == false) and ASuperSurvivor:usingGun() and
            (ASuperSurvivor:getDangerSeenCount() == 0) and
    ...
    if (getSpecificPlayer(0) == nil) or (not getSpecificPlayer(0):isAsleep()) then
        ...
        elseif (ASuperSurvivor:getCurrentTask() == "None") and (IsInBase) and (not IsInAction) and (ZombRand(4) == 0) then
            ...
            elseif (SurvivorsFindWorkThemselves) and not (AiNPC_Job_Is(NPC, "Guard")) and
                not (AiNPC_Job_Is(NPC, "Doctor")) and not (AiNPC_Job_Is(NPC, "Farming")) then
