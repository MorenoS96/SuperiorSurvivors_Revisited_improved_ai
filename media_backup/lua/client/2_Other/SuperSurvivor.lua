...
function SuperSurvivor:walkTowards(x,y,z)
	...
	local towardsSquare = getTowardsMiddle(self.player,x,y,z)
...
function SuperSurvivor:getMaxWeaponRange()
    ...
    if (self.player:getPrimaryHandItem() ~= nil) then
        if (instanceof(self.player:getPrimaryHandItem(), "HandWeapon")) then
            return self.player:getPrimaryHandItem():getMaxRange(self.player)
        end
    end
    return out
end
...
function SuperSurvivor:NPC_ShouldRunOrWalk()
	...
	if (distanceAlt < 10) then
		print("NPC_ShouldRunOrWalk set running to false due to distanceAlt < 10 SRW_0000")
		self:setRunning(false)
		return
	end
	...
	if (self.LastEnemeySeen ~= nil) then
		...
		if(self:Task_IsNotFleeOrFleeFromSpot())then
			self:setRunning(false)
            print("NPC_ShouldRunOrWalk set running to false due to Task_IsNotFleeOrFleeFromSpot returned true SRW_0001")
			return
		end
		if (distance < 20) or (zNPC_AttackRange) then
			self:setRunning(false)
			print("NPC_ShouldRunOrWalk set running to false due to distance < 2 or distanceAlt < 2 or zNPC_AttackRange true SRW_0002")
			return
		end
	end
	self:setRunning(true)
	print("NPC_ShouldRunOrWalk set running to true SRW_0002")
...
function SuperSurvivor:NPC_MovementManagement()
	if (self:isWalkingPermitted()) and (not self:hasGun()) then
		...
		local minrange = self:getMinWeaponRange()
		local zNPC_AttackRange = self:isEnemyInRange(self.LastEnemeySeen)
		local maxRange = self:getMaxWeaponRange()

		if (distance >= maxRange - 0.2 and not zNPC_AttackRange) then
			-- The actual walking itself
			self:setRunning(false)
			if(instanceof(self.LastEnemeySeen,"IsoPlayer")) then	
				self:walkTowards(cs:getX(),cs:getY(),cs:getZ())
			else
				local fs = cs:getTileInDirection(self.LastEnemeySeen:getDir())
				if(fs) and (fs:isFree(true)) then
					self:walkTowards(fs:getX(),fs:getY(),fs:getZ())
					self:DebugSay("AtkTicks NPC_MovementManagement Walkto FS")
				else 
					self:walkTowards(cs:getX(), cs:getY(), cs:getZ())
					self:DebugSay("AtkTicks NPC_MovementManagement Walkto CS")
...
function SuperSurvivor:getGunHitChange(weapon,victim)
	...
	local xpForAiming = self.player:getXp():getXP(Perks.FromString("Aiming"))
	print("Aiming XP : " .. tostring(xpForAiming))
	...
	print("Base hit chance : " .. tostring(hitChance))
	local distance = getDistanceBetween(self.player,victim)
	local finalHitChance = hitChance - distance
	print("Final hit chance : " .. tostring(finalHitChance))
	return finalHitChance -- TODO: change formula when coverValue != 0
end
