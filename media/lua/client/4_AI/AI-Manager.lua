...
local function AiNpc_IsInDanger(NPC)
    ...
    return ((hasWeapon and (dangerCount >= 1 or isEnemyInRange)) or
...
function AIManager(TaskMangerIn)
    ...
    -- print(NPC:getName()..": " ..tostring(ASuperSurvivor:needToFollow()))

    if ASuperSurvivor:needToFollow() and not CanShootFromVehicle(ASuperSurvivor) then
        return TaskMangerIn
    end
    ...
    if (Bravery == 0) then
        ASuperSurvivor:setBravePoints(4)
    ...
    if (ASuperSurvivor:getGroupRole() == "Companion") then
        ...
        if (ASuperSurvivor:needToFollow()) then
    ...
    if (AiNPC_Job_Is(NPC, "Companion")) then -- and (DistanceBetweenMainPlayer <= 12)) then
        ...
        -- Pursue
        -- ------------ --
        if AiNPC_TaskIsNot(AiTmi, "First Aide") and AiNPC_TaskIsNot(AiTmi, "Pursue") and
            AiNPC_TaskIsNot(AiTmi, "Attack") and AiNPC_TaskIsNot(AiTmi, "Flee") and
            (NPC.LastEnemeySeen ~= nil and Distance_AnyEnemy < NPC:NPC_CheckPursueScore()) then
        ...
        --	NPC:Companion_DoSixthSenseScan()
        -- ------------------------- --
        -- NPC:DebugSay("AM I CLOSE ENOUGH TO ATTACK?: "..tostring(( (getDistanceBetween(ASuperSurvivor.LastEnemeySeen,ImFollowingThisChar) > GFollowDistance) )))
        ...
        if ((TaskMangerIn:getCurrentTask() ~= "Attack") and (TaskMangerIn:getCurrentTask() ~= "Threaten") and
            (TaskMangerIn:getCurrentTask() ~= "First Aide") and (Task_Is_Not("Flee")) and
            (ASuperSurvivor:isInSameRoom(ASuperSurvivor.LastEnemeySeen)) ---	and (ASuperSurvivor:isInSameRoom(ASuperSurvivor.LastEnemeySeen)) 
        ...
        and (not NPC:isTooScaredToFight()) --	and (ASuperSurvivor:inFrontOfLockedDoor() == false) 
        then
            if (ASuperSurvivor.player ~= nil) and (ASuperSurvivor.player:getModData().isRobber) and
                (not ASuperSurvivor.player:getModData().hitByCharacter) and EnemyIsSurvivor and
                (not EnemySuperSurvivor.player:getModData().dealBreaker) then
                TaskMangerIn:AddToTop(ThreatenTask:new(ASuperSurvivor, EnemySuperSurvivor, "Scram"))
                ASuperSurvivor:DebugSay("Threaten/Attack Task condition triggered! Reference Number ATC_000_03")
            else
                TaskMangerIn:AddToTop(AttackTask:new(ASuperSurvivor))
                ASuperSurvivor:DebugSay("Threaten/Attack Task condition triggered! Reference Number ATC_000_04")
            end
        end
        ...
        -- --------------------------------- --
        if (ASuperSurvivor:getNeedAmmo()) and (ASuperSurvivor:hasAmmoForPrevGun()) then
        ...
        --  NPC:getDangerSeenCount() removed
        -- --------------------------------- --
        if (NPC:needToReadyGun(weapon)) then
            NPC:DebugSay(tostring(((NPC:needToReload()) or (NPC:needToReadyGun(weapon)))) .. "," ..
                             tostring(((ASuperSurvivor:hasAmmoForPrevGun()) or SurvivorInfiniteAmmo)) .. "," ..
                             tostring(NPC:usingGun()) .. "," .. tostring((ASuperSurvivor:getNeedAmmo())))
        end
        if ((NPC:needToReload()) or (NPC:needToReadyGun(weapon))) and
            ((ASuperSurvivor:hasAmmoForPrevGun()) or SurvivorInfiniteAmmo) and NPC:usingGun() -- removed and (ASuperSurvivor:getNeedAmmo() condition - 
        then
            NPC:ReadyGun(weapon)
        ...
        -- 	Equip Weapon                 -- 
        -- ----------------------------- --
        if (ASuperSurvivor:hasWeapon()) and (ASuperSurvivor:Get():getPrimaryHandItem() == nil) and
            (TaskMangerIn:getCurrentTask() ~= "Equip Weapon") then
        ...
        -- Flee to heal  -- 
        -- -----------   --
        if (TaskMangerIn:getCurrentTask() ~= "Flee") and (isRAMBO < 2) and
            (((NPC.EnemiesOnMe > CanTollerateEnemiesOnMe) and (NPC.dangerSeenCount > Bravery) and (NPC:hasWeapon()) and
                (not NPC:usingGun())) -- Melee
            or
                ((NPC.EnemiesOnMe > CanTollerateEnemiesOnMe) and (NPC.dangerSeenCount > Bravery) and (NPC:hasWeapon()) and
                    (NPC:usingGun())) -- Gun general
            or ((NPC.EnemiesOnMe > 0) and ((ASuperSurvivor:needToReload()) or (ASuperSurvivor:needToReadyGun(weapon)))) or
                (IHaveInjury and NPC.dangerSeenCount > 0) or (NPC.dangerSeenCount >= 5)) then
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if (ASuperSurvivor:Task_IsPursue_SC() == true) and (Distance_AnyEnemy <= 9) and
            (Distance_AnyEnemy < NPC:NPC_CheckPursueScore()) then
    ...
    if (getSpecificPlayer(0) ~= nil) then
        ...
        if ((TaskMangerIn:getCurrentTask() ~= "Surender") and (TaskMangerIn:getCurrentTask() ~= "Flee") and
            (TaskMangerIn:getCurrentTask() ~= "Flee From Spot") and (TaskMangerIn:getCurrentTask() ~= "Clean Inventory") and
            (SSM:Get(0) ~= nil and SSM:Get(0):usingGun()) and getSpecificPlayer(0) and
            getSpecificPlayer(0):CanSee(ASuperSurvivor.player) and (not ASuperSurvivor:usingGun() or
            (not ASuperSurvivor:RealCanSee(getSpecificPlayer(0)) and DistanceBetweenMainPlayer <= 3)) and
            getSpecificPlayer(0):isAiming() and IsoPlayer.getCoopPVP() and
            not ASuperSurvivor:isInGroup(getSpecificPlayer(0)) and (facingResult > 0.95) and
            (DistanceBetweenMainPlayer < 6)) then
            TaskMangerIn:clear()
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        ...
        if ((TaskMangerIn:getCurrentTask() ~= "Attack") and (TaskMangerIn:getCurrentTask() ~= "Threaten") and
            (TaskMangerIn:getCurrentTask() ~= "First Aide") and (Task_Is_Not("Flee")) and
            (ASuperSurvivor:isInSameRoom(ASuperSurvivor.LastEnemeySeen))) -- and ( (ASuperSurvivor:isEnemyInRange(ASuperSurvivor.LastEnemeySeen)))  -- this make them not move to engage zombie even a few tiles away when in follow mode? is that intentional
        and (ASuperSurvivor:getDangerSeenCount() > 0) -- cant attack what you don't see. must have seen an enemy in danger range to attack
        and (NPC:getCurrentTask() ~= "Follow" or
            (getDistanceBetween(ASuperSurvivor.LastEnemeySeen, ImFollowingThisChar) < (GFollowDistance + AttackRange))) -- move to engage an enemie only if they within follow range (when following)
        and (not NPC:isTooScaredToFight()) --	and (ASuperSurvivor:inFrontOfLockedDoor() == false) 
        then
            if (ASuperSurvivor.player ~= nil) and (ASuperSurvivor.player:getModData().isRobber) and
                (not ASuperSurvivor.player:getModData().hitByCharacter) and EnemyIsSurvivor and
                (not EnemySuperSurvivor.player:getModData().dealBreaker) then
                TaskMangerIn:AddToTop(ThreatenTask:new(ASuperSurvivor, EnemySuperSurvivor, "Scram"))
                ASuperSurvivor:DebugSay("Threaten/Attack Task condition triggered! Reference Number ATC_000_06")
            else
                TaskMangerIn:AddToTop(AttackTask:new(ASuperSurvivor))
                ASuperSurvivor:DebugSay("Threaten/Attack Task condition triggered! Reference Number ATC_000_05")
            end
        end
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if (TaskMangerIn:getCurrentTask() ~= "Find Building") and (TaskMangerIn:getCurrentTask() ~= "First Aide") and
            (TaskMangerIn:getCurrentTask() ~= "Flee") and ((IHaveInjury) and (ASuperSurvivor:isTooScaredToFight())) and
            (ASuperSurvivor:getDangerSeenCount() > 0) then
            TaskMangerIn:AddToTop(FindBuildingTask:new(ASuperSurvivor))
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if (IHaveInjury) then
            if (TaskMangerIn:getCurrentTask() ~= "First Aide") and (TaskMangerIn:getCurrentTask() ~= "Flee") and
                (TaskMangerIn:getCurrentTask() ~= "Doctor") and (TaskMangerIn:getCurrentTask() ~= "Hold Still") and
                (isRAMBO < 2) and ((NPC:getSeenCount() >= 1) and (Distance_AnyEnemy <= 6)) -- This line doesn't make sense, what if the npc needs to heal outside of hostiles?
            then
                TaskMangerIn:AddToTop(FirstAideTask:new(ASuperSurvivor)) -- If general healing
                TaskMangerIn:AddToTop(FleeTask:new(ASuperSurvivor))
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then -- To ABSOLUTELY prevent these two jobs from listening to this task.
        if (TaskMangerIn:getCurrentTask() ~= "Flee") and (TaskMangerIn:getCurrentTask() ~= "Flee From Spot") and
            (TaskMangerIn:getCurrentTask() ~= "Surender") and
            ((TaskMangerIn:getCurrentTask() ~= "Surender") and not EnemyIsSurvivor) and (isRAMBO < 2) and
            ((((NPC:needToReload()) or (NPC:needToReadyGun(weapon))) and
                ((NPC:getDangerSeenCount() > 1 and (Distance_AnyEnemy < 3) and (EnemyIsZombie)) or
                    ((NPC:getSeenCount() >= 2) and (Distance_AnyEnemy <= 2) and (EnemyIsZombie)))) -- AH HA, gun running away for non-companions when the npc is trying to reload or ready gun
            or (((NPC:needToReload()) or (NPC:needToReadyGun(weapon))) and
                ((NPC:getDangerSeenCount() > 1 and (Distance_AnyEnemy <= 2) and (EnemyIsSurvivor)) or
                    ((Distance_AnyEnemy <= 2) and (EnemyIsSurvivor)))) -- AH HA, gun running away for non-companions when the npc is trying to reload or ready gun
            ...
            or ((NPC.EnemiesOnMe > 3 and NPC:getDangerSeenCount() > 3 and NPC:getSeenCount() > 3) or
                (not ASuperSurvivor:hasWeapon() and (ASuperSurvivor:getDangerSeenCount() > 0)) or
                (IHaveInjury and ASuperSurvivor:getDangerSeenCount() > 0) or
                (EnemyIsSurvivorHasGun and ASuperSurvivor:hasGun() == false) or (ASuperSurvivor:isTooScaredToFight()))) then
            ...
            if ((TaskMangerIn:getCurrentTask() == "LootCategoryTask") or
                (TaskMangerIn:getCurrentTask() == "Pile Corpses")) then -- currently to dangerous to loot said building. so give up it
                ...
                if not (AiNPC_Job_Is(NPC, "Guard")) and not (AiNPC_Job_Is(NPC, "Doctor")) then
                    TaskMangerIn:AddToTop(FleeFromHereTask:new(ASuperSurvivor, ASuperSurvivor:Get():getCurrentSquare()))
                    ASuperSurvivor:DebugSay(
                        "Flee from too many zombies condition triggered! Reference Number LCT_000_02_REG")
                else
                    ASuperSurvivor:DebugSay(
                        "Flee from too many zombies condition triggered! Reference Number LCT_000_02_ALT")
                end
            end
    ...
    if ((TaskMangerIn:getCurrentTask() ~= "Flee") and (TaskMangerIn:getCurrentTask() ~= "Surender") and
        ((TaskMangerIn:getCurrentTask() ~= "Surender") and (not EnemyIsSurvivor))) and (isRAMBO < 2) then
        if ((NPC.EnemiesOnMe > CanTollerateEnemiesOnMe)) then
            -- print(tostring(NPC.EnemiesOnMe)..","..tostring(CanTollerateEnemiesOnMe)..","..tostring(ASuperSurvivor.LastEnemeySeen:GetAnimSetName()))
            TaskMangerIn:AddToTop(FleeTask:new(ASuperSurvivor))
            ASuperSurvivor:DebugSay("Companion FLEEINGTASK_0003")

        end
        if ((NPC.EnemiesOnMe > 0) and
            (ASuperSurvivor:usingGun() and ((ASuperSurvivor:needToReload()) or (ASuperSurvivor:needToReadyGun(weapon))))) then
            TaskMangerIn:AddToTop(FleeTask:new(ASuperSurvivor))
            ASuperSurvivor:DebugSay("Companion FLEEINGTASK_0004")
        end
        if (IHaveInjury and (NPC.dangerSeenCount > 0)) then
            TaskMangerIn:AddToTop(FleeTask:new(ASuperSurvivor))
            ASuperSurvivor:DebugSay("Companion FLEEINGTASK_0005")
        end
        if (NPC.dangerSeenCount > Bravery) then
            TaskMangerIn:AddToTop(FleeTask:new(ASuperSurvivor))
            ASuperSurvivor:DebugSay("Companion FLEEINGTASK_0002")
        end
    end

    --- end of high priority ai actions
    if (TaskMangerIn:getCurrentTask() == "Give Reward To Player") or
        (TaskMangerIn:getCurrentTask() == "Direct Dialogue") then -- if quest related action only allow high priority AI to interupt
        return TaskMangerIn
    end
    ...
    -- ----------------------------- --
    if (false) and (ASuperSurvivor:getAIMode() ~= "Random Solo") and
        ((ASuperSurvivor:isStarving()) or (ASuperSurvivor:isDyingOfThirst())) then

        -- leave group and look for food if starving
        ASuperSurvivor:DebugSay(
            "Left the building because starving or thirsty condition triggered! Reference Number 000_000_04")
        ...
        ASuperSurvivor:getTaskManager():clear()
        if (ASuperSurvivor:Get():getStats():getHunger() > 0.40) then
            ASuperSurvivor:Get():getStats():setHunger(0.40)
        end
        if (ASuperSurvivor:Get():getStats():getThirst() > 0.40) then
            ASuperSurvivor:Get():getStats():setThirst(0.40)
        end
        ASuperSurvivor:Speak(getDialogue("LeaveGroupHungry"))

    elseif (TaskMangerIn:getCurrentTask() ~= "Enter New Building") and
        (TaskMangerIn:getCurrentTask() ~= "Clean Inventory") and (IsInAction == false) and
        (TaskMangerIn:getCurrentTask() ~= "Eat Food") and (TaskMangerIn:getCurrentTask() ~= "Find This") and
        (TaskMangerIn:getCurrentTask() ~= "First Aide") and (TaskMangerIn:getCurrentTask() ~= "Listen") and
        (((ASuperSurvivor:isHungry()) and (IsInBase)) or ASuperSurvivor:isVHungry()) and
        (ASuperSurvivor:getDangerSeenCount() == 0) then

        if (not ASuperSurvivor:hasFood()) and (ASuperSurvivor:getNoFoodNearBy() == false) and
            ((getSpecificPlayer(0) == nil) or (not getSpecificPlayer(0):isAsleep())) then

            if (HisGroup) then
                local area = HisGroup:getGroupAreaCenterSquare("FoodStorageArea")
                if (area) then
                    ASuperSurvivor:DebugSay(
                        "Hunber/Thirst-Like Conditions Met in AImanager - Reference Number DHFB_0002")
    ...
    -- ----------------------------- --
    if (TaskMangerIn:getCurrentTask() ~= "Enter New Building") and (IsInAction == false) and
        (TaskMangerIn:getCurrentTask() ~= "Eat Food") and (TaskMangerIn:getCurrentTask() ~= "Find This") and
        (TaskMangerIn:getCurrentTask() ~= "First Aide") and
        (((ASuperSurvivor:isThirsty()) and (IsInBase)) or ASuperSurvivor:isVThirsty()) and
        (ASuperSurvivor:getDangerSeenCount() == 0) then
        if (ASuperSurvivor:getNoWaterNearBy() == false) and
            ((getSpecificPlayer(0) == nil) or (not getSpecificPlayer(0):isAsleep())) then
            if (HisGroup) then
                local area = HisGroup:getGroupAreaCenterSquare("FoodStorageArea")
                if (area) then
            ...
            ASuperSurvivor:DebugSay(
                "Not enter new building condition met! Condition number (to compare in ai manager file) 0004")
        end
    end
    ...
    -- 			Listen to Task
    -- ----------------------------- --
    if ((ASuperSurvivor:Get():getModData().InitGreeting ~= nil) or (ASuperSurvivor:getAIMode() == "Random Solo")) and
        (TaskMangerIn:getCurrentTask() ~= "Listen") and (TaskMangerIn:getCurrentTask() ~= "Surender") and
        (TaskMangerIn:getCurrentTask() ~= "Flee From Spot") and (TaskMangerIn:getCurrentTask() ~= "Take Gift") and
        (ASuperSurvivor.LastSurvivorSeen ~= nil) -- and (ASuperSurvivor.LastSurvivorSeen:isGhostMode() == false)
    and (ASuperSurvivor:getSpokeTo(ASuperSurvivor.LastSurvivorSeen:getModData().ID) == false) and
        (getDistanceBetween(ASuperSurvivor.LastSurvivorSeen, ASuperSurvivor:Get()) < 8) and
        (ASuperSurvivor:getDangerSeenCount() == 0) and (TaskMangerIn:getCurrentTask() ~= "First Aide") and
        (ASuperSurvivor:Get():CanSee(ASuperSurvivor.LastSurvivorSeen)) then
    ...
    -- ----------------------------- --
    if not (AiNPC_Job_Is(NPC, "Companion")) then
        if (ASuperSurvivor:getNeedAmmo()) and (ASuperSurvivor:hasAmmoForPrevGun()) and (IsInAction == false) and
            (TaskMangerIn:getCurrentTask() ~= "Take Gift") and (TaskMangerIn:getCurrentTask() ~= "Flee") -- New
        and (TaskMangerIn:getCurrentTask() ~= "Flee From Spot") -- New
        and (ASuperSurvivor:getDangerSeenCount() == 0) then
    ...
    if not (AiNPC_Job_Is(NPC, "Companion")) then -- removed and (ASuperSurvivor:getNeedAmmo() == false) condition as I dont remember what that is for
        if (IsInAction == false) and ASuperSurvivor:usingGun() and (ASuperSurvivor:getDangerSeenCount() == 0) and
            ((ASuperSurvivor:needToReload()) or (ASuperSurvivor:needToReadyGun(weapon))) and
            (NPC:NPC_FleeWhileReadyingGun()) then
    ...
    if ((getSpecificPlayer(0) == nil) or (not getSpecificPlayer(0):isAsleep())) and
        (ASuperSurvivor:getAIMode() ~= "Stand Ground") then
        ...
        if (ASuperSurvivor:getGroupRole() == "Guard") then
            ...
            if (Task_Is_Not("Attack") and Task_Is_Not("Threaten") and Task_Is_Not("Pursue") and Task_Is_Not("Flee") and
                Task_Is_Not("First Aide") and Task_Is_Not("Find This") and Task_Is_Not("Eat Food") and
                Task_Is_Not("Follow") and (IsInAction == false)) then

                if (HisGroup:getGroupAreaCenterSquare("GuardArea") ~= nil) and (HisGroup:getGroupArea("GuardArea")) then
                    if (getDistanceBetween(HisGroup:getGroupAreaCenterSquare("GuardArea"), NPC:Get():getCurrentSquare()) >
                        10) then
                        TaskMangerIn:clear()
                ...
                if (HisGroup:getGroupAreaCenterSquare("GuardArea") == nil) and (CenterBaseSquare ~= nil) and
                    not (IsInBase) then
                ...
                elseif (HisGroup:getGroupAreaCenterSquare("GuardArea") == nil) and (CenterBaseSquare == nil) and
                    not (IsInBase) then
                ...
                if Task_Is("Flee") then
                    NPC:NPC_ShouldRunOrWalk()
                end
                if (NPC:isSpeaking() == false) then
                    NPC:DebugSay("GuardTask Cond_0005")
                end
            end
        ...
        if (not SurvivorsFindWorkThemselves or not IsInBase) and (ASuperSurvivor:getGroupRole() == "Guard") and
            (ASuperSurvivor:getCurrentTask() == "None") and (not IsInAction) and (ZombRand(4) == 0) then
        ...
        elseif (ASuperSurvivor:getCurrentTask() == "None") and (IsInBase) and (not IsInAction) and (ZombRand(4) == 0) then
            ...
            if (not SurvivorsFindWorkThemselves) and (ASuperSurvivor:getGroupRole() == "Doctor") then
                ...
                if (randresult == 1) then
                    ...
                    if (medicalarea) and (medicalarea[1] ~= 0) then
                        gotoSquare = getCenterSquareFromArea(medicalarea[1], medicalarea[2], medicalarea[3],
                            medicalarea[4], medicalarea[5])
                    end
                    ...
                    if (gotoSquare) then
                        ASuperSurvivor:walkTo(gotoSquare)
                    end
            ...
            elseif (SurvivorsFindWorkThemselves) and not (AiNPC_Job_Is(NPC, "Guard")) and
                not (AiNPC_Job_Is(NPC, "Leader")) and not (AiNPC_Job_Is(NPC, "Doctor")) and
                not (AiNPC_Job_Is(NPC, "Farming")) then
                ...
                if (ASuperSurvivor:Get():getBodyDamage():getWetness() < 0.2) then
                    ...
                    if (SafeToGoOutAndWork) then
                        ...
                        jobScores["Chop Wood"] = 2 +
                                                     math.min(
                                ASuperSurvivor:Get():getPerkLevel(Perks.FromString("Axe")), 3)
                        jobScores["Forage"] = 2 +
                                                  math.min(
                                ASuperSurvivor:Get():getPerkLevel(Perks.FromString("Foraging")), 3)

                        -- deprioritize assigned tasks
                        jobScores["Farming"] = 0 +
                                                   math.min(
                                ASuperSurvivor:Get():getPerkLevel(Perks.FromString("Farming")), 3)
                        jobScores["Doctor"] = -2 +
                                                  math.min(
                                ASuperSurvivor:Get():getPerkLevel(Perks.FromString("Doctor")), 3) +
                                                  math.min(
                                ASuperSurvivor:Get():getPerkLevel(Perks.FromString("First Aid")), 3)
                        jobScores["Guard"] = 2 +
                                                 math.min(ASuperSurvivor:Get():getPerkLevel(Perks.FromString("Aiming")),
                                3)
                        ...
                        if (chopWoodSquare == nil) then
                            jobScores["Chop Wood"] = -10
                        end
                        if (farmingArea[1] == 0) then
                            jobScores["Farming"] = -10
                        end
                        if (guardArea[1] == 0) then
                            jobScores["Guard"] = -10
                        end
                        ...
                        for key, value in pairs(jobScores) do
                            ...
                            if value >= jobScores[job] then
                                job = key
                            end
                        end

                        ASuperSurvivor:Get():getStats():setBoredom(
                            ASuperSurvivor:Get():getStats():getBoredom() + (ZombRand(5) / 100.0))
                        if (job == "Relax") then
                            ASuperSurvivor:Speak(getActionText("IGoRelax"))
                        ...
                        elseif (job == "Gather Wood") then
                            ...
                            if (woodstoragearea[1] ~= 0) then
                                dropSquare = getCenterSquareFromArea(woodstoragearea[1], woodstoragearea[2],
                                    woodstoragearea[3], woodstoragearea[4], woodstoragearea[5])
                            end
                        ...
                        elseif (job == "Pile Corpses") then
                            ...
                            if (storagearea[1] ~= 0) then
                                dropSquare = getCenterSquareFromArea(storagearea[1], storagearea[2], storagearea[3],
                                    storagearea[4], storagearea[5])
                            end
                            if (dropSquare) then
                        ...
                        elseif (job == "Forage") then
                            ...
                            if (FoodStorageCenter) then
                                dropSquare = FoodStorageCenter
                            end
        ...
        if (ASuperSurvivor:getCurrentTask() == "None") and (IsInBase == false) and (not IsInAction) and
            (HisGroup ~= nil) then
    ...
    if (ASuperSurvivor:getAIMode() == "Random Solo") and (TaskMangerIn:getCurrentTask() ~= "Listen") and
        (TaskMangerIn:getCurrentTask() ~= "Take Gift") then -- solo random survivor AI flow	

        if (TaskMangerIn:getCurrentTask() == "None") and (ASuperSurvivor.TargetBuilding ~= nil) and
            (not ASuperSurvivor:getBuildingExplored(ASuperSurvivor.TargetBuilding)) and
            (not ASuperSurvivor:isEnemyInRange(ASuperSurvivor.LastEnemeySeen)) then
            TaskMangerIn:AddToTop(AttemptEntryIntoBuildingTask:new(ASuperSurvivor, ASuperSurvivor.TargetBuilding))
            ASuperSurvivor:DebugSay(
                "Attempt entry into building Task condition met in AI manager! Reference number B_0001")

            --		V backup just in case
            --		elseif(TaskMangerIn:getCurrentTask() == "None") then
        elseif (TaskMangerIn:getCurrentTask() == "None") and
            ((not EnemyIsSurvivor) or (not ASuperSurvivor:isEnemyInRange(ASuperSurvivor.LastEnemeySeen))) then
        ...
        if (ASuperSurvivor.TargetBuilding ~= nil) or (ASuperSurvivor:inUnLootedBuilding()) then
            ...
            if (not ASuperSurvivor:hasWeapon()) and (TaskMangerIn:getCurrentTask() ~= "Loot Category") and
                (ASuperSurvivor:getDangerSeenCount() <= 0) and (ASuperSurvivor:inUnLootedBuilding()) and
                (NPC:isTargetBuildingClaimed(ASuperSurvivor.TargetBuilding) == false) then
            ...
            elseif (ASuperSurvivor:hasRoomInBag()) and (TaskMangerIn:getCurrentTask() ~= "Loot Category") and
                (ASuperSurvivor:getDangerSeenCount() <= 0) and (ASuperSurvivor:inUnLootedBuilding()) and
                (NPC:isTargetBuildingClaimed(ASuperSurvivor.TargetBuilding) == false) then
        ...
        if (SurvivorBases) and (IsInAction == false) and -- New. Hopefully to stop spam
        (ASuperSurvivor:getBaseBuilding() == nil) and (ASuperSurvivor:getBuilding()) and
            (TaskMangerIn:getCurrentTask() ~= "First Aide") and (TaskMangerIn:getCurrentTask() ~= "Attack") and
        ...
        (TaskMangerIn:getCurrentTask() ~= "Enter New Building") and -- new
            (TaskMangerIn:getCurrentTask() ~= "Barricade Building") and (ASuperSurvivor:hasWeapon()) and
        ...
        if ((SurvivorBases) and (ASuperSurvivor:isStarving()) or (ASuperSurvivor:isDyingOfThirst())) and
            (ASuperSurvivor:getBaseBuilding() ~= nil) then -- leave group and look for food if starving
            ...
            print(ASuperSurvivor:getName() .. ": clearing task manager because too hungry")
            ASuperSurvivor:resetAllTables()
            ASuperSurvivor:setBaseBuilding(nil)
            if (ASuperSurvivor:Get():getStats():getHunger() > 0.30) then
                ASuperSurvivor:Get():getStats():setHunger(0.30)
            end
            if (ASuperSurvivor:Get():getStats():getThirst() > 0.30) then
                ASuperSurvivor:Get():getStats():setThirst(0.30)
            end
