...
function SuperSurvivor:new(isFemale,square)
	...
	setmetatable(o, self)
	self.__index = self
	o.SwipeStateTicks = 0 -- used to check if survivor stuck in the same animation frame
	o.UpdateDelayTicks = 20
	o.DebugMode = false
	...
	o.Bikuri = TextDrawObject.new()	
	o.Bikuri:setAllowAnyImage(true);
	o.Bikuri:setDefaultFont(UIFont.Large);
	o.Bikuri:setDefaultColors(255, 255, 0, 255);
	o.Bikuri:ReadString("!")
	
	o.NoResultActions = {}
	o.YesResultActions = {}
	o.ContinueResultActions = {}
	o.HasQuestion = false
	o.HasBikuri = false
	o.TriggerName = ""
...
function SuperSurvivor:isTooScaredToFight() -- This function is an absolute mess...
	if (not self:HasMultipleInjury() and not self:usingGun() ) then 
		return false
	end
		return (self.EnemiesOnMe > 1) 
end
function SuperSurvivor:usingWeapon()
	local handItem = sself.player:getUseHandWeapon()
...
function SuperSurvivor:usingGun()
	local handItem = self.player:getUseHandWeapon()
	if(handItem ~= nil) then
		local isFireArm=handItem:isAimedFirearm() or handItem:isRanged() or handItem:getMaxRange() > 2.0
		return isFireArm
	end
	return false
end
...
function SuperSurvivor:needToFollow()
	...
	--[[   Nolan: this stuff should really go into the FollowTask.needToFollow function.
	local DistanceBetweenMainPlayer = getDistanceBetween(getSpecificPlayer(0),self:Get()) 
	
	if  ( 	-- Checks for NPC and main player is indoor/outdoor-like conditions
			( (DistanceBetweenMainPlayer > 5) and    (    getSpecificPlayer(0):isOutside() and not self:Get():isOutside() ) ) -- Player Outside / NPC Inside
		 or ( (DistanceBetweenMainPlayer > 5) and    (not getSpecificPlayer(0):isOutside() and     self:Get():isOutside() ) ) -- Player Inside / NPC Outside
...
function SuperSurvivor:CanShootFromVehicle()
	return CanShootFromVehicle(self.player)
end
...
function SuperSurvivor:DoVision()
	...
	if(self.dangerSeenCount > 0) and (self:getCurrentTask() == "Attack") then self.UpdateDelayTicks = 10 -- only do it when fighting so they dont follow or flee slowly when not fighting
	else self.UpdateDelayTicks = 20 end
...
--	self.LastSurvivorSeen = nil
	...
	if(spottedList ~= nil) then
		...
		--print(self:getName()..": EOM: "..tostring(self.EnemiesOnMe))
	end
...
function SuperSurvivor:updateTime()
	...
	if(self.Reducer % self.UpdateDelayTicks == 0) then  -- the lower the number you put here the more frequent survivor:update() gets called, means faster reactions but poorer performance
...
function SuperSurvivor:HasMultipleInjury()
	...
	for i=0, bodyparts:size()-1 do
		...
		if(bp:HasInjury()) and (not bp:bandaged() ) then
			total = total + 1
...
function SuperSurvivor:addXp(perkString, amount)
    if self.player and self.player:getXp() and perkString and amount then
        local perk = Perks.FromString(perkString)
        if perk then
            print("adding xp: " .. tostring(amount) .. " to perk: " .. tostring(perkString))
            self.player:getXp():AddXP(perk, amount, false, true, true)
        end
    end
...
--[[
	self.player:setIdleAnimatorState()
	
	self.player:clearVariable("TimedActionType");
	self.player:clearVariable("BumpFallType");
	self.player:clearVariable("WeaponReloadType");
	self.player:setPerformingAnAction(false)
	self.player:clearVariable("bdoshove")
	self.player:clearVariable("isattacking")
	self.player:clearVariable("AttackAnim")
	self.player:clearVariable("bShoveAiming");
	self.player:clearVariable("bShoveAiming");
	]]
...
function SuperSurvivor:Attack(victim)
	...
	if(self:WeaponReady()) then
		...
		if(self.player ~= nil) then 
			local distance = getDistanceBetween(self.player,victim)
			local minrange = self:getMinWeaponRange() + 0.1	-- ZombRand(0,5)	If you want random chance, remove the number and put the ZombRand in.
			local weapon = self.player:getPrimaryHandItem();
			...
			if(distance < minrange) or (self.player:getPrimaryHandItem() == nil) then
				...
				if (self.AtkTicks <= 0) then -- First to make sure it's okay to attack
					...
					--	 print("---------")
						local atkTicks = weapon:getSwingTime()
						if(atkTicks>1)	then
							atkTicks=1
						end
						self:getSoundManager():playSound(weapon:getSwingSound());
						-- Added RealCanSee to see if it works | and (damage > 0)
						if (hitChance >= dice) and (damage > 0) and (self:RealCanSee(victim)) then
							victim:Hit(weapon, self.player, damage, false, 1.0, false)
							self:addXp("Aiming", damage*10);
							print("I HIT THE GUNSHOT!")
							self.AtkTicks = atkTicks
						else
							self:addXp("Aiming", 10);
...
function SuperSurvivor:getSoundManager()
    return self.player:getEmitter()
end
