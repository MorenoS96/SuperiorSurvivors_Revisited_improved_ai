...
function FollowTask:needToFollow()
    ...
    if self.Complete == true or self.parent == nil or self.FollowChar == nil or self.FollowChar:getCurrentSquare() ==
        nil then
        return false
    end

    local distance = getDistanceBetween(self.parent.player, self.FollowChar)
    if (distance > GFollowDistance + self.FollowDistanceOffset) or
        (self.parent:getBuilding() ~= self.FollowChar:getCurrentSquare():getBuilding()) or
        self.parent:Get():getVehicle() or (self.FollowChar:getVehicle() ~= self.parent:Get():getVehicle()) then
...
function FollowTask:update()
    ...
    if (true) then -- self.parent:isInAction() == false) then -- for some reason this is true when they doing nothing sometimes...
        ...
        -- old code
        if (distance > (GFollowDistance + self.FollowDistanceOffset + 5)) or
            (self.FollowChar:getVehicle() ~= self.parent:Get():getVehicle()) then
            self.parent:setRunning(true)
        else
            self.parent:setRunning(false)
        end
        ...
        if (self.FollowChar:getZ() > self.parent.player:getZ()) and
            (self.parent:isInSameBuilding(self.FollowChar) == false) then
        ...
        if not ropeSquare then
            ...
            if (distance > (GFollowDistance + self.FollowDistanceOffset)) and (self.parent:Get():getVehicle() == nil) then
                ...
                if (gotosquare ~= nil) then
                    if (gotosquare:getRoom() ~= nil) and (gotosquare:getRoom():getBuilding() ~= nil) then
            ...
            elseif (self.FollowChar:getVehicle() ~= nil) and (self.parent:Get():getVehicle() == nil) then
                ...
                local towed_car = car:getVehicleTowing()
                local numOfSeats = car:getScript():getPassengerCount()

                local all_occupied = true
                for i = 0, numOfSeats - 1 do
                    if not car:isSeatOccupied(i) then
                        all_occupied = false
                        break
                    end
                end
                if (all_occupied) then
                    car = towed_car
                    numOfSeats = car:getScript():getPassengerCount()
                end
                self.MySeat = -1
                local doorseat = -1
                ...
                for seat = numOfSeats - 1, 1, -1 do
                    ...
                    if (not tempDoor) then
                        tempDoor = car:getPassengerDoor2(seat)
                    end
                    ...
                    if (tempDoor ~= nil) then
                        ...
                        if (car:isSeatOccupied(seat) == false) then
                            doorseat = seat
                        end
                    end
                    ...
                    if (doorseat ~= -1) and (self.MySeat ~= -1) then
                        break
                    end
                end
                ...
                if (self.MySeat ~= -1) then
                    ...
                    if (doorsquare ~= nil) then
                        ...
                        print(self.parent:getName() .. ": adding enter vehicle timed actions and waiting, dist:" ..
                                  tostring(distance))
                        ...
                        if (distance > 3) then
                            ...
                            self.parent:StopWalk()
                            ISTimedActionQueue.add(ISEnterVehicle:new(self.parent:Get(), car, 1))
                            if (self.MySeat ~= 1) then
