...
function AttackTask:isComplete()
    ...
    if (not self.parent:needToFollow()) and ((self.parent:getDangerSeenCount() > 0) or
        ...
        -- self.parent.player:AttemptAttack(10);
        return true
    end
end
...
function AttackTask:isValid()
    ...
    if (not self.parent) or (not self.parent.LastEnemeySeen) or
        (not self.parent:isInSameRoom(self.parent.LastEnemeySeen) and not self.parent:CanShootFromVehicle()) or
        (self.parent.LastEnemeySeen:isDead()) then
        print("Attack task is not valid")
        return false
    else
        return true
    end
